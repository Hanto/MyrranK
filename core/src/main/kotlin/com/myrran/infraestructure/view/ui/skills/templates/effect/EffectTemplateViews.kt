...
): Container<Table>(), Identifiable<SkillViewId>, Observer<SkillEvent>, Disposable
{
    ...
    private fun updateQuantities() {
        ...
        model.learnedEffectTemplates().forEach { views[it.value.id]?.setAvailable(it) }
    }
    ...
    override fun propertyChange(event: SkillEvent) {
        ...
        when (event) {
            ...
            is EffectSkillChangedEvent -> updateQuantities()
            is EffectSkillRemovedEvent -> updateQuantities()
        }
    }
    ...
    private fun createEffectTemplateViews(): Map<EffectTemplateId, EffectTemplateView> =

        model.learnedEffectTemplates()
            .associate { it.value.id to factory.createEffectTemplateView(it) }
}
